...
// Tool result type for simple text results
 ...
 */
function formatResult(content: string, details: TextToolResult): AgentToolResult<TextToolResult> {
	return {
...
export const writeTool: AgentTool = {
	...
	async execute(_toolCallId, params: any) {
		...
		try {
			...
			const details: TextToolResult = { path: filePath, size: content.length };
			if (backupPath) details.backup = backupPath;
...
export const bashTool: AgentTool = {
	...
	async execute(_toolCallId, params: any) {
		...
		try {
			...
			const details: TextToolResult = { command, exitCode: 0, duration, category };
