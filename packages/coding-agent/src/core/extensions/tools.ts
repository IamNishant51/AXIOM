...
import { getExtensionRegistry, type ExtensionDefinition } from "./registry.js";
...
export const addExtensionTool: AgentTool = {
	...
	label: "Add Extension",
	description:
		"Add a new tool/extension to the agent. Creates tools for tasks like generating passwords, sending emails, API calls, etc. Code runs in a sandbox with access to: fs, path, os, crypto modules.",
	...
	async execute(_toolCallId, params: any): Promise<AgentToolResult<ExtensionResult>> {
		...
		try {
			...
			return formatResult(
				`Extension "${extension.label}" (${extension.name}) added successfully.`,
				{
					name: extension.name,
					action: "added",
					details: {
...
export const reloadExtensionsTool: AgentTool = {
	...
	parameters: Type.Object({}),
	async execute(): Promise<AgentToolResult<ExtensionResult>> {
		const registry = getExtensionRegistry();
		...
		return formatResult(
			`Extensions reloaded. ${loadedExtensions.length} custom extension(s) active.`,
			{
				name: "extensions",
				action: "reloaded",
