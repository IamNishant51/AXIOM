...
): AgentToolResult<ExtensionResult> {
	return {
 ...
 */
export const addExtensionTool: AgentTool = {
	name: "add_extension",
	label: "Add Extension",
	description:
		"Add a new tool/extension to the agent. When the user asks to add a tool that does something specific, use this tool to create it. The code runs in a sandboxed context with access to: fs, path, os, crypto modules.",
	parameters: Type.Object({
		name: Type.String({
			description:
				"Unique name for the extension (lowercase, underscores allowed)",
		}),
		...
		parameters: Type.Record(Type.String(), Type.Any(), {
			description:
				"JSON Schema for the extension's parameters (e.g., { message: { type: 'string', description: 'The message' } })",
		}),
		code: Type.String({
			description:
				"The JavaScript code to execute. Use 'params' object for input, return an object with 'content' (array of {type: 'text', text: string}) and 'details' properties. Access modules via context.require('module'). Available modules: fs, path, os, crypto",
		}),
	}),
	async execute(_toolCallId, params: any): Promise<AgentToolResult<ExtensionResult>> {
		...
		try {
			...
			if (registry.hasTool(name)) {
				throw new Error(
					`Extension "${name}" already exists. Use remove_extension first to replace it.`,
				);
			}
			...
			return formatResult(
				`Extension "${extension.label}" (${extension.name}) has been added successfully. You can now use it.`,
				{
					name: extension.name,
					action: "added",
					details: {
			...
			throw new Error(
				`Failed to add extension: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	},
};
 ...
 */
export const listExtensionsTool: AgentTool = {
	name: "list_extensions",
	label: "List Extensions",
	description:
		"List all installed extensions/tools. Use this to see what custom tools are available.",
	parameters: Type.Object({}),
	async execute(_toolCallId): Promise<AgentToolResult<any>> {
		const registry = getExtensionRegistry();
		...
		const output = [
			...
			...extensionList.map(
				(ext) =>
					`  - ${ext.name}: ${ext.label} - ${ext.description} (created: ${ext.createdAt})`,
			),
			"",
 ...
 */
export const removeExtensionTool: AgentTool = {
 ...
 */
export const reloadExtensionsTool: AgentTool = {
	...
	parameters: Type.Object({}),
	async execute(_toolCallId): Promise<AgentToolResult<ExtensionResult>> {
		const registry = getExtensionRegistry();
		...
		const tools = registry.getAllTools().filter(
			(t) =>
				t.name !== "add_extension" &&
		...
		return formatResult(
			`Extensions reloaded. ${tools.length} extension tool(s) are now active.`,
			{
				name: "extensions",
				action: "reloaded",
 ...
 */
export const getExtensionTool: AgentTool = {
	name: "get_extension",
	label: "Get Extension",
	...
	async execute(_toolCallId, params: any): Promise<AgentToolResult<any>> {
		...
		return formatResult(
			`Extension: ${extension.label} (${extension.name})
Description: ${extension.description}
 ...
 */
export const extensionTools: AgentTool[] = [
	addExtensionTool,
	listExtensionsTool,
	removeExtensionTool,
