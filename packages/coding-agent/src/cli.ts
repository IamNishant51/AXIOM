...
Be helpful, concise, and proactive. When the user asks for code, provide working solutions.`;
	 ...
	 * Create new session
	 */
	private async createSession(initialPrompt?: string): Promise<string> {
	 ...
	 * Set up TUI
	 */
	private setupTui(): void {
		const theme = {
			borderColor: (s: string) => s, // Simple for now
		};
		...
		if (this.agent) {
			...
			try {
				...
				if (lastMessage && lastMessage.role === "assistant") {
					const textContent = lastMessage.content.find((c: any) => c.type === "text") as any;
					if (textContent?.text) {
						this.tui.addChild(new Spacer(1));
...
`;
	 ...
	 */
	async runNonInteractive(prompt: string): Promise<void> {
		if (!this.agent) {
