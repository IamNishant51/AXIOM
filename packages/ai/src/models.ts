...
import type { Api, KnownApi, KnownProvider, Model, Provider } from "./types.js";
...
// Registry storage
const providers = new Map<Provider, ProviderMeta>();
const apis = new Map<Api, (model: Model<any>, context: any, options?: any) => AsyncIterable<any>>();

// Built-in model data (simplified - real version would have hundreds)
const BUILT_IN_MODELS: Model<any>[] = [
	// Anthropic
	{
...
export function initializeProviders(): void {
	...
	const providerModels = new Map<Provider, Model<any>[]>();
...
export function getProviders(): Provider[] {
	if (providers.size === 0) initializeProviders();
...
export function getModels(provider: string): Model<any>[] {
	if (providers.size === 0) initializeProviders();
	const meta = providers.get(provider as Provider);
	return meta?.models || [];
}
