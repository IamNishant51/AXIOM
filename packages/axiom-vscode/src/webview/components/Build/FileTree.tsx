...
function getFileIcon(filename: string) {
  ...
  return FILE_ICONS[ext] || <File className="w-4 h-4 text-axiom-text-muted" />;
}
    ...
    <>
      ...
      >
        ...
        <span className="w-4 flex-shrink-0">
          {kind === 'dir' && (
            isOpen ? (
              <ChevronDown className="w-4 h-4 text-axiom-text-muted" />
            ) : (
              <ChevronRight className="w-4 h-4 text-axiom-text-muted" />
            )
          )}
        </span>
        ...
        <span className="flex-shrink-0">
          {kind === 'dir' ? (
            isOpen ? (
              <FolderOpen className="w-4 h-4 text-axiom-accent-blue" />
            ) : (
              <Folder className="w-4 h-4 text-axiom-accent-blue" />
            )
          ) : (
            getFileIcon(name)
          )}
        ...
        <span className={`
          ...
          ${kind === 'dir' ? 'text-axiom-text-primary font-medium' : 'text-axiom-text-secondary'}
        `}>
          {name}
          {kind === 'dir' && '/'}
        </span>
...
function EmptyFiles() {
  return (
    ...
    >
      ...
      >
        <Folder className="w-8 h-8 text-axiom-text-muted" />
      </motion.div>
